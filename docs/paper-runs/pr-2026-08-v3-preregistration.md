...
ends and a new one begins — with two documented exceptions below.

**Computing the Git commit pin.** Do not pin raw `git log -1 --format=%H` (current
HEAD). Sealing this very document is itself a commit; the moment it lands (and its
merge, if sealed via PR), HEAD moves past whatever it pinned — a self-referential trap
discovered on the `pr-2026-08-v2` seal, whose pin (`5a4654b...`) went stale the instant
its own sealing commit (`92d9381` → merge `07adc6c`) landed on the same branch. Pin
the **last commit that touched a runtime-safety path** instead — immune to docs-only
commits landing on top of it, including this document's own:

```bash
git log -1 --format=%H -- bridge.py guard.py monitor.py ibkr_operator.py \
