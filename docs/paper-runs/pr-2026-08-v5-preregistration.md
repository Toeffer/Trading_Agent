# Paper Run Pre-Registration — `pr-2026-08-v5`
...
> **Sealed:** not yet
>
> **Supersedes `pr-2026-08-v4`.** v4's own seal remains intact (verified: SHA-256
> `fda7628a261dcc25f4a13be6f0b7cb913a05cb26b67834cd4bafaf6b7dd328f9`, matching
> `pr-2026-08-v4-seal.json`) — it is not amended, only superseded, per its own
> immutability rule. It stopped being usable as the operative pre-registration not
> because anything in it was wrong, but because its Git runtime-safety pin went stale
> before the run started: PRs #10 and #11 (2026-08-13) made real fixes to `guard.py`
> and `ibkr_operator.py` — both enumerated runtime-safety paths — so the pathspec-
> filtered pin `preregistration-pin-verify` computes moved from `ff79733...` to
> `3a8d722...`. Sections 1, 3, and 5 are unchanged from v4 — no strategy or risk logic
> was touched — only §2's Git runtime-safety pin differs.
>
> Run ID `pr-2026-08-v5` is a placeholder Werner picked to name this file — rename the
...
| Git runtime-safety pin | `3a8d7220d1b6cb404d1f901ce28f0103b960099b` |
