# Paper Run Pre-Registration — `pr-2026-08-v6`
...
> **Sealed:** not yet
>
> **Supersedes `pr-2026-08-v5`.** Same root cause as the v4→v5 supersession, not a
> new problem: PR #13 (2026-08-17, heartbeat systemd timer tracking + alerting)
> again touched `ibkr_operator.py` and `monitor.py` — both enumerated runtime-safety
> paths — moving the pathspec-filtered pin from `3a8d722...` to `0e6b3f5...`, hours
> before this run's own start date (2026-08-17). The pin is doing exactly its job:
> catching the runtime-safety baseline moving before the window opens, not after.
>
> Neither superseded document was amended. v5's seal remains intact (verified:
> SHA-256 `e688ca53be07f78d3fd7e3890efd1289eeaa5ac35a34e3594f93d138e01cd358`, matching
> `pr-2026-08-v5-seal.json`); v4's likewise (SHA-256
> `fda7628a261dcc25f4a13be6f0b7cb913a05cb26b67834cd4bafaf6b7dd328f9`, matching
> `pr-2026-08-v4-seal.json`). Sections 1, 3, and 5 are byte-identical across v4, v5,
> and v6 — no strategy or risk logic has changed at any point; only §2's Git
> runtime-safety pin has moved, each time in step with a real, deliberate fix to the
> tooling that checks it.
>
> Run ID `pr-2026-08-v6` is a placeholder Werner picked to name this file — rename the
...
| Git runtime-safety pin | `0e6b3f5b3a19ff3262a5842e25bfb55b8dd6327a` |
