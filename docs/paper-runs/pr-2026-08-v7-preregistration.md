# Paper Run Pre-Registration — `pr-2026-08-v7`
...
> **Sealed:** not yet
>
> **Supersedes `pr-2026-08-v6`.** Same recurring root cause as v4→v5 and v5→v6, not
> a new problem: PR #15 (2026-08-17, a real Tier-1 fix — preflight's calendar
> rollover was raising an unhandled PermissionError against the H1-protected
> guard-state.json write, discovered live on this run's own start day and fixed the
> same day) touched `guard.py`, an enumerated runtime-safety path, moving the
> pathspec-filtered pin from `0e6b3f5...` to `1ce10bc...`. The pin caught the
> baseline moving before the window opens, exactly as designed.
>
> No prior superseded document was amended. v6's seal remains intact (verified:
> SHA-256 `b56dfe6f53e782c7686d79cdba21d72e0c14b7d1691704344072fb4576b36b17`, matching
> `pr-2026-08-v6-seal.json`); v5's and v4's likewise (SHA-256
> `e688ca53be07f78d3fd7e3890efd1289eeaa5ac35a34e3594f93d138e01cd358` and
> `fda7628a261dcc25f4a13be6f0b7cb913a05cb26b67834cd4bafaf6b7dd328f9` respectively).
> Sections 1, 3, and 5 are byte-identical across v4 through v7 — no strategy or risk
> logic has changed at any point; only §2's Git runtime-safety pin has moved, each
> time in step with a real, deliberate fix to the tooling or safety logic that
> checks it. This one — unlike the tooling-only moves behind v5 and v6 — is a
> genuine `guard.py` safety-path fix, and required a bridge restart to take effect
> (unlike the prior two, which only affected fresh CLI invocations).
>
> Run ID `pr-2026-08-v7` is a placeholder Werner picked to name this file — rename the
...
| Git runtime-safety pin | `1ce10bc89f68772d7b1616fd4f9def571988b1ab` |
