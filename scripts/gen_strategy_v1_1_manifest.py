...
manifest: dict = {
    ...
    "design_summary": {
        "universe_expansion_proposed": {
            ...
            "effective_breadth": {
                ...
                "directional_component": {
                    "note": "Raw return correlations; long-only equity beta dominates",
                    "v1_0_0": {"n": 4, "rho_bar": 0.65, "n_eff": 1.36},
                    "v1_1": {"n": 22, "rho_bar": 0.45, "n_eff": 2.11},
                    "relative_ir": 1.24,
                },
                "selection_component": {
                    "note": "Beta-residual correlations; cross-sectional relative strength",
                    "v1_0_0": {"n": 4, "residual_rho_bar": 0.40, "n_eff": 1.82},
                    "v1_1": {"n": 22, "residual_rho_bar": 0.20, "n_eff": 4.23},
                    "relative_ir": 1.52,
                },
                "conclusion": "The allowlist expansion buys selection breadth, not diversification of market risk. Market risk is controlled by the regime gate and inverse-vol scalar.",
                "superseded_claim": {"n_eff": 6.5, "relative_ir": 2.24,
                                     "note": "Asserted in proposal versions 0.1 to 0.3; overstated the gain by roughly 80 percent; corrected in 0.4"},
                "correlations_are_reference_estimates": True,
            },
            "market_cap_verified_symbols": 14,
            "market_cap_unverified_symbols": ["AVGO", "LLY", "PG", "HD", "CAT", "UNP", "NEE", "DUK"],
            "market_cap_verification_note": "14 of 22 verified against live data 2026-07-31; smallest verified UNH ~376B and KO ~377B, far above the 10B threshold. Remaining 8 gated by the market-data plan; complete before 19B applies the list.",
            "applied": False,
        ...
        "gate_i_sector_cap_proposed": {
            ...
            "applies_to": "BUY",
            "value_rationale": "1 not 2: momentum clusters by sector, so a cap of 2 would let the relative-strength filter fill two slots with one bet. Near-duplicate pairs in the universe: NVDA/AMD, META/GOOGL, JPM/BAC, XOM/CVX.",
            "sector_group_count": 11,
            "indirect_position_ceiling": 11,
            "sell_exempt": True,
            "fail_mode": "closed",
            "implemented": False,
    ...
    "non_us_equity_policy": {
        ...
        "resolved": "2026-08-01",
        "admission_criteria": [
            "Listed on a US exchange and USD-denominated",
            "isAdr false — ordinary shares, not depositary receipts",
            "Issuer domiciled in the approved jurisdiction set",
            "No VIE or equivalent contractual-control structure",
        ],
        "approved_jurisdictions": ["US", "CA", "GB", "IE", "NL", "DE", "FR", "CH",
                                   "SE", "DK", "NO", "FI", "AU", "NZ", "JP", "IL"],
        "jurisdiction_principle": "Developed markets with a US tax treaty and PCAOB audit access; the list is an instantiation of this principle",
        "rejected_pure_venue_reading_because": "Would admit Chinese VIE structures and HFCAA-exposed issuers on the same rule that admits a Dutch cloud provider",
        "rejected_pure_domicile_reading_because": "Excludes a large class of legitimate developed-market issuers for no risk-based reason",
        "enforcement": {
            "mechanism": "Gate A allowlist admission criterion, not a runtime gate",
            "guard_has_domicile_check": False,
            "verified": "2026-08-01 — no domicile, country, ISIN or ADR check exists in guard.py; gate_allowlist is pure membership and fails closed",
            "principle": "Code enforces legally mandated exclusions (H4.1/PRIIPs); the allowlist enforces preferences",
            "contract_level_check_rejected_because": "Tier-1 guard change requiring contract data that may be unavailable at preflight, introducing a fail-open path on lookup failure",
        },
        "fpi_reporting_caveat": "Foreign Private Issuers file 20-F annually and often report semi-annually. Irrelevant to v1.1 (signals are entirely price-based) but would degrade any future Hermes filing or earnings-language sleeve.",
        "tax_treatment": "Out of scope for this document. Dividend withholding and US estate-tax situs differ for foreign-issuer shares. Moot for IREN and NBIS (no dividends) but applies to future candidates. Requires a tax advisor, not Werner.",
    },
    ...
    "symbols_considered_and_not_admitted": [
        {"symbol": "IREN", "domicile": "AU", "isin": "AU0000185993", "exchange": "NASDAQ Global Select",
         ...
         "status": "ELIGIBLE_FOR_CONSIDERATION_V1_2",
         "meets_11_7_criteria": True,
         "admitted": False,
         "outstanding_checks": [
             "BTC-proxy test: rolling correlation and beta versus BTC and versus NVDA",
             "Market-cap stability: clears 10B at 13.1B today, but a 52-week low of 14.72 against a 76.87 high implies a sub-10B cap within the year; needs an explicit measurement rule",
         ],
         "note": "Meets all four section 11.7.2 admission criteria; NASDAQ-listed, USD, isAdr false. An earlier characterisation of IREN as primarily a Bitcoin miner rested on vendor description prose and is WITHDRAWN as unverified — the same payload classifies it as Information Technology Services. The trading-relevant question is whether the equity still trades as a BTC proxy, which requires rolling BTC correlation and beta; price-history and revenue-segmentation endpoints are gated on the current market-data plan."},
        {"symbol": "NBIS", "domicile": "NL", "isin": "NL0009805522", "exchange": "NASDAQ Global Select",
         ...
         "meets_11_7_criteria": True,
         "admitted": False,
         "note": "Formerly Yandex N.V. Meets all four section 11.7.2 admission criteria."},
    ...
    "follow_up_obligations": [
        ...
        "Apply the section 11.7.2 admission criteria to strategy_v1.md section 3 at promotion, replacing the bare Non-US equities row",
