"""Tests for Phase 16C — Manual Level-1 Promotion Dry-Run Gate.

All tests are read-only. No broker mutation, no order endpoints,
no H1 token usage, no autonomy level changes, no promotion performed,
no order enablement performed.
...
Coverage:
  ...
  - JSON stdout pure
  - Export written
  - Clean runtime => level1_dry_run_gate_ready / OK
  - Missing 16B tag => NO_GO
  ...
  - promotion_performed=false always
  - order_enablement_allowed_now=false always
  - order_enablement_performed=false always
  - future command previews are strings only
  - no /order* calls
  - no H1 token reads
...
from ibkr_operator import (
    ...
    _run_level1_promotion_dry_run_gate,
    _PHASE16C_DIAGNOSIS,
    _PHASE16C_REQUIRED_TAGS,
    _PHASE16C_EXPORT_DIR,
...
# ===========================================================================
# Fixtures — same style as 16B tests
# ===========================================================================
...
def clean_git_metadata():
    return {
        ...
        "tag": "phase16b_manual_level1_promotion_review",
    }
...
def all_tags_present():
    return {
        ...
        "present_count": len(_PHASE16C_REQUIRED_TAGS),
        "missing": [],
        "present": list(_PHASE16C_REQUIRED_TAGS),
    }
...
def one_tag_missing():
    return {
        "required_count": len(_PHASE16C_REQUIRED_TAGS),
        "present_count": len(_PHASE16C_REQUIRED_TAGS) - 1,
        "missing": [list(_PHASE16C_REQUIRED_TAGS)[0]],
        "present": list(_PHASE16C_REQUIRED_TAGS)[1:],
    }
...
class TestCommandExists:
    ...
    @pytest.mark.parametrize("alias", [
        "manual-level1-dry-run-gate",
        "phase16c-promotion-dry-run",
        "level1-gate-dossier",
    ])
    ...
    def test_function_importable(self):
        assert callable(_run_level1_promotion_dry_run_gate)


# ===========================================================================
# T2: Clean runtime => level1_dry_run_gate_ready / OK
# ===========================================================================
...
class TestCleanRuntime:
    def test_clean_runtime_produces_ok(self,
        ...
        try:
            ...
            assert result["diagnosis"] == _PHASE16C_DIAGNOSIS["ready"]
            ...
            # Dry-run gate
            dg = result["dry_run_gate"]
            assert dg["gate_ready"] is True
            assert dg["promotion_dry_run_only"] is True
            assert dg["promotion_performed"] is False
            assert dg["order_enablement_performed"] is False
            assert dg["requires_future_explicit_human_apply"] is True
            assert "Phase 16D" in dg.get("proposed_next_step", "")
            # Future command previews are strings
            assert isinstance(dg.get("future_apply_command_preview"), str)
            assert isinstance(dg.get("rollback_command_preview"), str)
            assert isinstance(dg.get("relock_command_preview"), str)
            # Dry-run steps
            drs = result["dry_run_steps"]
            assert drs["step_count"] == 8
            assert all(s.get("performed") is False for s in drs["steps"])
            # Future Level-1 controls
            flc = result["future_level1_controls"]
            assert flc["max_autonomy_level"] == "1"
            assert flc["advisory_only_hermes"] is True
            assert flc["manual_approval_required"] is True
            assert flc["h1_required_for_approve_submit"] is True
            assert flc["preflight_required"] is True
            assert flc["post_promotion_relock_required"] is True
            # Prerequisites
...
class TestMissingTags:
    def test_missing_16b_tag_produces_no_go(self,
                                            clean_git_metadata,
                                            clean_worktree, origin_aligned,
        ...
        try:
            result = _run_level1_promotion_dry_run_gate()
            assert result["diagnosis"] == _PHASE16C_DIAGNOSIS["missing_required_tags"]
            assert result["severity"] == "NO_GO"
            assert result["operator_action_required"] is True
            assert result["dry_run_gate"]["gate_ready"] is False
        finally:
            stop_patches(mocks, patches)
...
class TestDirtyWorktree:
    def test_dirty_worktree_produces_no_go(self,
        ...
        try:
            result = _run_level1_promotion_dry_run_gate()
            assert result["diagnosis"] == _PHASE16C_DIAGNOSIS["dirty_worktree"]
            assert result["severity"] == "NO_GO"
        finally:
            stop_patches(mocks, patches)
...
class TestDisconnected:
    def test_disconnected_produces_hold(self,
        ...
        try:
            result = _run_level1_promotion_dry_run_gate()
            assert result["diagnosis"] == _PHASE16C_DIAGNOSIS["runtime_not_ready"]
...
class TestSafetyUnlocked:
    def test_env_allow_orders_true(self, clean_git_metadata,
        ...
        try:
            result = _run_level1_promotion_dry_run_gate()
            assert result["diagnosis"] == _PHASE16C_DIAGNOSIS["safety_not_locked"]
            assert result["severity"] == "NO_GO"
        finally:
            stop_patches(mocks, patches)
...
class TestGuardState:
    def test_nonzero_trade_count(self, clean_git_metadata,
        ...
        try:
            result = _run_level1_promotion_dry_run_gate()
            assert result["diagnosis"] == _PHASE16C_DIAGNOSIS["guard_state_not_clean"]
            assert result["severity"] == "NO_GO"
        finally:
            stop_patches(mocks, patches)
...
class TestActiveAlerts:
    def test_active_alerts_no_go(self, clean_git_metadata,
        ...
        try:
            result = _run_level1_promotion_dry_run_gate()
            assert result["diagnosis"] == _PHASE16C_DIAGNOSIS["monitor_alerts_active"]
            assert result["severity"] == "NO_GO"
        finally:
            stop_patches(mocks, patches)
...
class TestPositionsNotFlat:
    def test_non_flat_positions_no_go(self, clean_git_metadata,
        ...
        try:
            result = _run_level1_promotion_dry_run_gate()
            assert result["diagnosis"] == _PHASE16C_DIAGNOSIS["positions_not_flat"]
            assert result["severity"] == "NO_GO"
        finally:
            stop_patches(mocks, patches)
...
class TestJsonOutput:
    def test_json_parseable(self, clean_git_metadata,
        ...
        try:
            ...
            assert parsed["gate_id"] == result["gate_id"]
        finally:
            stop_patches(mocks, patches)
...
# ===========================================================================
# T13: Future command previews are strings only
# ===========================================================================

class TestFutureCommandPreviews:
    def test_previews_are_strings_not_executed(self, clean_git_metadata,
                                               clean_worktree, origin_aligned,
                                               all_tags_present, bridge_health_ok,
                                               positions_flat, alerts_clean,
                                               snapshot_ok, readiness_locked,
        ...
        try:
            result = _run_level1_promotion_dry_run_gate()
            dg = result["dry_run_gate"]
            assert isinstance(dg["future_apply_command_preview"], str)
            assert "autonomy-promotion-plan" in dg["future_apply_command_preview"]
            assert isinstance(dg["rollback_command_preview"], str)
            assert "target-level 0" in dg["rollback_command_preview"]
            assert isinstance(dg["relock_command_preview"], str)
            assert "safety-relock" in dg["relock_command_preview"]
            # These are constants, never executed
            assert dg["future_apply_command_preview"] == _PHASE16C_FUTURE_APPLY_COMMAND_PREVIEW
            assert dg["rollback_command_preview"] == _PHASE16C_ROLLBACK_COMMAND_PREVIEW
            assert dg["relock_command_preview"] == _PHASE16C_RELOCK_COMMAND_PREVIEW
        finally:
            stop_patches(mocks, patches)
    ...
    def test_no_forbidden_endpoints(self):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name == "_run_level1_promotion_dry_run_gate":
                ...
                return
        pytest.fail("Could not find dry-run gate function")
...
class TestNoMutation:
    def test_no_mutation(self, clean_git_metadata,
        ...
        try:
            ...
            assert len(result["explicit_non_actions"]) >= 6
        finally:
            stop_patches(mocks, patches)
