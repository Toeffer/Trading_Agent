...
import logging
from trading_agent.api import install as _install_execution_api
from trading_agent.runtime import Runtime as _ExecutionRuntime
from trading_agent.settings import Settings as _ExecutionSettings
from trading_agent.compatibility import ReadOnlyBrokerProxy as _ReadOnlyBrokerProxy
from fastapi import Request
from trading_agent.diagnostic_workers import DiagnosticWorkers

from dotenv import load_dotenv
from fastapi import FastAPI, HTTPException, Header
from pydantic import BaseModel, ConfigDict

# Environment is supplied by the service manager. No configuration file reads
# or account/state mutations take place during module import.
...
async def _start_execution_runtime():
    global ib, _startup_safety
    runtime = _ExecutionRuntime(_ExecutionSettings.from_env())
    runtime.start()
    app.state.runtime = runtime
    app.state.diagnostic_workers = DiagnosticWorkers()
    ib = _ReadOnlyBrokerProxy(runtime.owner)
    _startup_safety = {"ok": True, "state": "initialized", **runtime.settings.public_identity()}


@app.on_event("shutdown")
async def _stop_execution_runtime():
    runtime = getattr(app.state, "runtime", None)
    if runtime is not None:
        runtime.close()
        app.state.diagnostic_workers.close()
...
if _IBKR_BRIDGE_DEBUG:
    ...
    def _m():
        d={}
        try:
            for l in open(f"/proc/{os.getpid()}/status", encoding="utf-8"):
...
async def _startup_ibkr_autoconnect():
    await _startup_auto_connect()


# ---------------------------------------------------------------------------
# Startup auto-connect — bounded read-only connect on service start
# ---------------------------------------------------------------------------
# One bounded connection attempt runs on the broker owner loop at startup.
# Failure requires manual connection and reconciliation. The historical helper
# name remains for compatibility; no HTTP self-call or retry loop is used.
_STARTUP_CONNECT_MAX_ATTEMPTS = 1
_STARTUP_CONNECT_RETRY_DELAY = 0.0


def _startup_self_connect_http() -> dict:
    return app.state.runtime.broker.connect()


async def _startup_auto_connect():
    try:
        await asyncio.to_thread(_startup_self_connect_http)
    except Exception:
        logging.getLogger('ibkr-bridge').warning('Initial connection unavailable; manual connection and reconciliation required')
...
ib = None  # Assigned a read-only proxy at explicit application startup.
...
# _internal_fetch_quote_safe. pass  # Shared bounded worker pool is closed only at application shutdown. leaves the
...
def _track_leaked_md_thread():
    ...
    pass  # Shared bounded worker pool is closed only at application shutdown. in _internal_fetch_quote_safe."""
...
def _run_startup_safety() -> dict:
    ...
    gs_content = None
    try:
        gs_content = json.loads(gs_path.read_text(encoding="utf-8"))
        gs_readable = True
    ...
    ge_line_count = 0
    try:
        for line in ge_path.read_text(encoding="utf-8").splitlines():
            line = line.strip()
            if line:
                json.loads(line)
                ge_line_count += 1
    ...
    if sa_path.exists():
        try:
            sa_content = json.loads(sa_path.read_text(encoding="utf-8"))
...
_startup_safety = {"ok": False, "state": "not_started"}
...
def _internal_fetch_quote_safe(symbol: str, timeout: float = _MARKET_SNAPSHOT_TIMEOUT) -> dict:
    ...
    """
    executor = app.state.diagnostic_workers
    try:
        ...
        pass  # Shared bounded worker pool is closed only at application shutdown.
...
def _internal_fetch_bars_safe(symbol: str, timeout: float = _MARKET_SNAPSHOT_TIMEOUT) -> list:
    ...
    try:
        ...
        try:
            ...
            # thread after pass  # Shared bounded worker pool is closed only at application shutdown.), same warning
...
def _internal_place_order(approval_record: dict) -> dict:
    raise RuntimeError("LEGACY_ORDER_PROVIDER_DISABLED: use the durable execution service")


def _cancel_parent_safe(order_id: int) -> bool:
    raise RuntimeError("MANUAL_BROKER_CANCELLATION_REQUIRED")
...
def status_dashboard() -> Dict[str, Any]:
    ...
    try:
        ...
        gc = subprocess.run(["git", "rev-parse", "HEAD"],
                            capture_output=True, text=True, cwd=BRIDGE_DIR, timeout=5, encoding="utf-8")
        if gc.returncode == 0:
...
# Register the durable application last, replacing historical mutation routes.
_install_execution_api(app)
